...
struct ParallelOrchestration {
    static func getShippingStatus(for user: User) -> Observable<ShippingStatus> {
        return AsyncDAOFacade.getOrderIds(for: user)
            // calls to AsyncDAOFacade.getOrder are expensive, so we dispatch each of them to their own threads
            .flatMap(AsyncDAOFacade.getOrder).observeOn(ConcurrentDispatchQueueScheduler(qos: .userInitiated))
            .flatMap { (order) in
